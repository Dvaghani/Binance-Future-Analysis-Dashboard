  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Top Driver */}
        {data.top_driver ? (
          <div className="card-white p-5 border-emerald-200/80 bg-emerald-50/20">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-emerald-700 flex items-center gap-1.5 uppercase tracking-wider">
        ...
        {/* Top Dragger */}
        {data.top_dragger ? (
          <div className="card-white p-5 border-rose-200/80 bg-rose-50/20">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-rose-700 flex items-center gap-1.5 uppercase tracking-wider">
                <AlertTriangle className="w-4 h-4 text-rose-600" />
                Primary Performance Drag
              </span>
              <span className="text-xs font-mono font-bold text-rose-700 bg-rose-100/80 px-2 py-0.5 rounded">
      ...
      <div className="card-white p-5">
        <h3 className="text-sm font-bold text-slate-900 mb-1">Net Realized PNL by Symbol</h3>
        <p className="text-xs text-slate-400 mb-4">Comparison of cumulative net profits and losses across traded contracts</p>

        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.assets} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F1F5F9" />
              <XAxis dataKey="symbol" tick={{ fontSize: 11, fill: '#475569' }} tickLine={false} axisLine={false} />
              <YAxis
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-white border border-slate-200 shadow-hover p-3 rounded-lg text-xs space-y-1">
                        <div className="font-bold text-slate-900">{d.symbol}</div>
                        <div
                        ...
                        <div className="text-slate-500">Win Rate: {d.win_rate}% ({d.trades} trades)</div>
                        <div className="text-slate-500">Profit Factor: {d.profit_factor.toFixed(2)}</div>
                      </div>
                    );
                  }
                  return null;
      ...
      <div className="card-white overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Per-Symbol Detailed Statistics</h3>
            <p className="text-xs text-slate-400">Granular volume, fee impact, and expectancy by traded pair</p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 font-mono">
              {data.assets.map((asset) => {
                ...
                return (
                  <tr key={asset.symbol} className="hover:bg-slate-50/80 transition">
                    <td className="py-3 px-4 font-sans font-bold text-slate-900">{asset.symbol}</td>
                    <td className={`py-3 px-4 font-bold tabular-nums ${isPos ? 'text-emerald-600' : 'text-rose-600'}`}>
                      {formatPNL(asset.net_pnl, 2)}
                    </td>
                    <td className="py-3 px-4 text-slate-700">{asset.win_rate}%</td>
                    <td className="py-3 px-4 text-slate-700">{asset.trades}</td>
                    <td className="py-3 px-4 text-emerald-600 font-semibold">{formatPNL(asset.avg_win, 2)}</td>
                    <td className="py-3 px-4 text-rose-600 font-semibold">{formatPNL(-Math.abs(asset.avg_loss), 2)}</td>
                    <td className="py-3 px-4 text-slate-700">{asset.profit_factor.toFixed(2)}</td>
                    <td className="py-3 px-4 text-rose-600 font-semibold text-[11px]">-{formatFee(asset.fees)}</td>
                    <td className={`py-3 px-4 font-semibold text-[11px] ${asset.funding >= 0 ? 'text-emerald-600' : 'text-slate-600'}`}>
                      {asset.funding >= 0 ? '+' : ''}{formatFee(asset.funding)}
                    </td>
                    <td className="py-3 px-4 text-slate-500">${asset.volume.toLocaleString()}</td>
